...
def hf_chunk_transcribe(audio_file, timestamp_text, transcription_text):
    ...
    chunks = hf_process_chunks(result["chunks"])
...
def olmoasr_seq_transcribe(audio_file, timestamp_text, transcription_text):
    ...
    chunks = olmoasr_process_chunks(result["segments"])
...
def hf_seq_transcribe(audio_file, timestamp_text, transcription_text):
    ...
    chunks = hf_seq_process_chunks(result["chunks"])
...
def olmoasr_process_chunks(chunks):
    ...
    for chunk in chunks:
        ...
        if not re.match(
            r"(foreign|foreign you|Thank you for watching!|you|you there)\s*$", text
        ):
            if text.strip() == "":
                continue
            start = chunk["start"]
...
def hf_process_chunks(chunks):
    ...
    for chunk in chunks:
        text = chunk["text"].strip()
        if not re.match(r"(foreign|foreign you|you|you there)\s*$", text):
            if text.strip() == "":
                continue
