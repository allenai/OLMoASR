...
def train(
    ...
    while True:
        ...
        with autocast(device_type="cuda"):
            ...
            start_gen = time.time()
            audio_files = ["/mmfs1/gscratch/efml/hvn2002/random_audio.npy" for i in range(train_batch_size)]
            transcript_files = ["/mmfs1/gscratch/efml/hvn2002/random_transcript.srt" for i in range(train_batch_size)]
            padded_audio_arr = [np.random.randint(low=-32768, high=32768, size=480000, dtype=np.int16) for i in range(train_batch_size)]
            audio_input = torch.empty((train_batch_size, 80, 3000), dtype=torch.float16).uniform_(-1.0, 1.0)
            text_input = torch.randint(0, 51864, (train_batch_size, 448), dtype=torch.long)
            text_y = torch.randint(0, 51864, (train_batch_size, 448), dtype=torch.long)
            padding_mask = torch.zeros((train_batch_size, 448, 448), dtype=torch.float16)
            end_gen = time.time()
            
            gen_time = (end_gen - start_gen) / 60
            gen_time_tensor = torch.tensor(gen_time, device=rank)
            if rank == 0:
                gathered_gen_time = [
                    torch.zeros_like(gen_time_tensor).to(rank)
                    for _ in range(dist.get_world_size())
                ]
            else:
                gathered_gen_time = None
            
            dist.gather(gen_time_tensor, gather_list=gathered_gen_time, dst=0)
            
            if rank == 0:
                gathered_gen_time = [t.item() for t in gathered_gen_time]
                for i, gen_time in enumerate(gathered_gen_time):
                    wandb.log({f"train/rand_gen_time_per_iter_gpu={i}": gen_time})
        ...
        backward_time = (end_backward - start_backward) / 60
        forward_time_tensor = torch.tensor(forward_time, device=rank)
        backward_time_tensor = torch.tensor(backward_time, device=rank)
        # prepare list to gather throughput from all processes
        if rank == 0:
            ...
            gathered_backward_time = [
                torch.zeros_like(backward_time_tensor).to(rank) 
                for _ in range(dist.get_world_size())
            ]
        else:
            gathered_forward_time = None
            gathered_backward_time = None

        dist.gather(forward_time_tensor, gather_list=gathered_forward_time, dst=0)
        dist.gather(backward_time_tensor, gather_list=gathered_backward_time, dst=0)

        if rank == 0:
            gathered_forward_time = [t.item() for t in gathered_forward_time]
            gathered_backward_time = [t.item() for t in gathered_backward_time]
            for i, forward_time in enumerate(gathered_forward_time):
                wandb.log({f"train/forward_time_per_iter_gpu={i}": forward_time}) 
            
            for i, backward_time in enumerate(gathered_backward_time):
                wandb.log({f"train/backward_time_per_iter_gpu={i}": backward_time})
