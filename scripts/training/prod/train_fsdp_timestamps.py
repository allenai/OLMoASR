...
def setup(rank: int) -> None:
    """Initializes the distributed process group for FSDP training.

    Sets the CUDA device for the current process and initializes the NCCL
    process group for distributed training with FSDP.
...
def prepare_dataloader(
    ...
    """Prepares the distributed dataloader for FSDP training.
...
def prepare_data(
    ...
    """Prepares the training dataset and dataloader for FSDP training.
...
def prepare_sched(
    ...
    """Prepares the learning rate scheduler for FSDP training.
...
def setup_wandb(
    ...
    """Sets up Weights and Biases logging for FSDP training.

    Initializes W&B with comprehensive training configuration and defines
    custom metrics for tracking training progress, efficiency, and validation.
...
def load_ckpt(
    ...
    model = olmoasr.model.OLMoASR(dims=train_state["dims"]).to(rank)
...
def train(
    ...
    eval_on_gpu: bool,
) -> Tuple[int, int, int, FSDP, torch.optim.Optimizer, Optional[GradScaler], LambdaLR]:
    """Main FSDP training loop with automatic mixed precision and distributed training.

    Performs forward pass, backward pass, gradient accumulation, and optional validation/evaluation.
    Includes comprehensive logging of training metrics, timing, and examples to W&B.
    Handles checkpointing and supports both synchronous validation and asynchronous evaluation.
    ...
    Args:
        ...
        scaler: Optional gradient scaler for mixed precision training (FSDP specific)
        model: FSDP-wrapped model for distributed training
        ...
        run_val: Whether to run validation during training
        val_freq: Frequency (in steps) for running validation
        val_batch_size: Batch size for validation
        val_num_workers: Number of workers for validation dataloader
        val_cache_dir: Directory for caching validation datasets
        val_wandb_log: Whether to log validation results to W&B
    ...
    Returns:
        ...
        - epoch: Current epoch number
        - model: The trained FSDP model
        - optimizer: Updated optimizer state
        - scaler: Updated gradient scaler state (if used)
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        with autocast(device_type="cuda", dtype=precision):
            ...
            # Forward pass through the FSDP model
            start_fwd = time.time()
        ...
        if scaler is not None:
            scaler.scale(train_loss).backward()  # Use sharded grad scaler for FSDP
        else:
            train_loss.backward()  # Standard backward pass
        end_bwd = time.time()
...
class ValidationDataset(Dataset):
    """Dataset for validation using HuggingFace speech recognition datasets.

    Supports multiple validation datasets including TED-LIUM, VoxPopuli, Common Voice,
    and AMI. Handles audio preprocessing (resampling, padding) and text tokenization
    with proper formatting for Whisper model evaluation.

    Attributes:
        val_set: Name of the validation dataset
        n_text_ctx: Maximum text context length
        dataset: Loaded HuggingFace dataset
    """

    def __init__(self, n_text_ctx: int, val_set: str, hf_token: str, cache_dir: str):
        """Initialize validation dataset.

        Args:
            n_text_ctx: Maximum number of text context tokens
            val_set: Name of the HuggingFace dataset to use for validation
            hf_token: HuggingFace authentication token
            cache_dir: Directory for caching downloaded datasets
        """
        # Configuration mapping for supported validation datasets
        valset2config = {
            "LIUM/tedlium": {
                "name": "release3",
                "split": "validation",
    ...
    def __getitem__(self, index):
        ...
        Extracts audio and text from the dataset, handles dataset-specific field names,
        and preprocesses both audio and text for model evaluation.
        ...
        Returns:
            Tuple containing:
            - audio_arr: Raw audio array
            - audio_input: Mel spectrogram tensor
            - text_input: Tokenized input text tensor
            - text_y: Tokenized target text tensor
            - padding_mask: Attention padding mask
        """
        global tokenizer
        ...
        if self.val_set == "LIUM/tedlium":
            ...
            waveform = self.dataset[index]["audio"]["array"]
            sampling_rate = self.dataset[index]["audio"]["sampling_rate"]
            text_y = self.dataset[index]["raw_text"]  # VoxPopuli uses 'raw_text'
            audio_arr, audio_input = self.preprocess_audio(waveform, sampling_rate)
            text_input, text_y, padding_mask = self.preprocess_text(text_y)
        elif self.val_set == "mozilla-foundation/common_voice_5_1":
            waveform = self.dataset[index]["audio"]["array"]
            sampling_rate = self.dataset[index]["audio"]["sampling_rate"]
            text_y = self.dataset[index]["sentence"]  # Common Voice uses 'sentence'
            audio_arr, audio_input = self.preprocess_audio(waveform, sampling_rate)
            text_input, text_y, padding_mask = self.preprocess_text(text_y)
    ...
    def preprocess_audio(
        ...
        """Preprocess audio waveform for validation.

        Resamples audio to 16kHz if needed, pads/trims to 30 seconds,
        and computes log mel spectrogram for model input.

        Args:
            waveform: Raw audio waveform array
            sampling_rate: Original sampling rate of the audio

        Returns:
            Tuple containing:
            - audio_arr: Processed audio array (30s at 16kHz)
            - audio_input: Log mel spectrogram tensor
        """
        # Resample to 16kHz if necessary (Whisper's expected sampling rate)
    ...
    def preprocess_text(
        ...
        """Preprocess text for validation.

        Tokenizes text using Whisper tokenizer, handles empty text with no-speech token,
        and creates input/target sequences with proper padding and masking.
        ...
        """
        # Handle empty text with special no-speech token
        if text.strip() == "":
            tokens = (
                list(tokenizer.sot_sequence_including_notimestamps)
                + [tokenizer.no_speech]
            ...
            # Normal text tokenization with SOT and EOT tokens
            tokens = (
                list(tokenizer.sot_sequence_including_notimestamps)
        ...
        # Create input/output sequences for teacher forcing
        text_input = tokens[:-1]  # All tokens except last
        text_y = tokens[1:]  # All tokens except first

        # Create attention mask to prevent attending to padding
        padding_mask = torch.zeros((self.n_text_ctx, self.n_text_ctx))
        padding_mask[:, len(text_input) :] = -np.inf
...
def validate(
    ...
    """Run validation on multiple HuggingFace speech datasets.

    Evaluates the FSDP model on validation sets including TED-LIUM, VoxPopuli,
    Common Voice, and AMI. Computes validation loss and WER metrics, logging
    results to W&B with detailed examples.

    Args:
        model: FSDP-wrapped model to validate
        precision: Mixed precision dtype for autocast
        tokenizer: Whisper tokenizer for text processing
        normalizer: Text normalizer for WER calculation
        rank: Global rank of current process
        local_rank: Local rank (GPU device ID)
        global_step: Current training step for logging
        batch_size: Batch size for validation
        num_workers: Number of dataloader workers
        n_text_ctx: Maximum text context length
        cache_dir: Directory for caching validation datasets
        wandb_log: Whether to log detailed examples to W&B
    """
    val_sets = [
        "LIUM/tedlium",
        "facebook/voxpopuli",
...
def run_async_eval(
    ...
    """Launch asynchronous evaluation subprocess for FSDP training.
...
def main(
    ...
    """Main training function for FSDP Whisper model training with timestamps.

    Orchestrates the complete FSDP training pipeline including:
    - Distributed training setup with FSDP sharding strategies
    - Data loading from compressed JSONL files
    - Model initialization with FSDP wrapping and activation checkpointing
    - Training loop with gradient accumulation and mixed precision
    - Periodic validation on HuggingFace datasets and asynchronous evaluation
    - Comprehensive logging to Weights & Biases

    Supports various FSDP sharding strategies, mixed precision training modes,
    and resume functionality from previous training runs.
    ...
    Args:
        ...
        val_batch_size: Batch size per process for validation
        num_workers: Number of worker processes for data loading
        prefetch_factor: Number of samples prefetched by each worker
        pin_memory: Whether to pin memory for faster GPU transfer
        shuffle: Whether to shuffle training data each epoch
        persistent_workers: Whether to keep workers alive between epochs
        run_val: Whether to run periodic validation during training
        val_freq: Frequency (in steps) for running validation
        val_wandb_log: Whether to log validation results to W&B
        ...
        ckpt_freq: Frequency (in steps) for saving checkpoints
        verbose: Whether to enable verbose model output
        precision: Mixed precision mode ("bfloat16", "float16", "pure_float16", "float32")
        hardware: Hardware type for FLOPS calculation ("H100", "A100", "L40", "A6000")
        eval_script_path: Path to evaluation script (for async evaluation)
        eval_wandb_log: Whether to log evaluation results to W&B
        eval_on_gpu: Whether to run evaluation on GPU (vs CPU)
        sharding_strategy: FSDP sharding strategy ("FULL_SHARD", "SHARD_GRAD_OP", etc.)
    """
