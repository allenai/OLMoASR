from whisper import audio, utils, DecodingOptions
...
from scripts import EvalAudioTextDataset
    ...
    for epoch in range(epochs):
                    ...
                    if (batch_idx + 1) % 10 == 0:
                        with open(
                            ...
                            for i, (tgt_text_instance, pred_text_instance) in enumerate(
                                ...
                                # logging to wandb table after 20 steps
                                if (batch_idx + 1) == 20:
                                    wer = np.round(
                                        ow.utils.calculate_wer(
                                            (tgt_text_instance, pred_text_instance)
        ...
        if (epoch + 1) % 2 == 0:
            if rank == 0:
                ...
                with open(f"logs/training/epoch_times_{'_'.join(tags)}.txt", "a") as f:
                    f.write(
                        f"eval epoch {epoch} took {(end_time - start_time) / 60.0} minutes\n"
                    )
