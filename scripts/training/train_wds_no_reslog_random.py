...
def train(
    ...
    while True:
        ...
        with autocast():
            ...
            start_gen = time.time()
            audio_files = [
                "/mmfs1/gscratch/efml/hvn2002/random_audio.npy"
                for i in range(train_batch_size)
            ]
            transcript_files = [
                "/mmfs1/gscratch/efml/hvn2002/random_transcript.srt"
                for i in range(train_batch_size)
            ]
            padded_audio_arr = [
                np.random.randint(low=-32768, high=32768, size=480000, dtype=np.int16)
                for i in range(train_batch_size)
            ]
            audio_input = torch.empty(
                (train_batch_size, 80, 3000), dtype=torch.float16
            ).uniform_(-1.0, 1.0)
            text_input = torch.randint(
                0, 51864, (train_batch_size, 448), dtype=torch.long
            )
            text_y = torch.randint(0, 51864, (train_batch_size, 448), dtype=torch.long)
            padding_mask = torch.zeros(
                (train_batch_size, 448, 448), dtype=torch.float16
            )
            end_gen = time.time()
