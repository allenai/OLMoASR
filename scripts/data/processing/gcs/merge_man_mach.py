...
def main():
    ...
    if not os.path.exists("id_to_machgen_shard.json.gz"):
        print("Downloading id_to_machgen_shard.json.gz")
        download_from_gcs(
            bucket_name="ow-seg",
            source_blob_name="id_to_machgen_shard.json.gz",
            destination_file_name="id_to_machgen_shard.json.gz",
        )

    print("Loading id_to_machgen_shard.json.gz")
    with gzip.open("id_to_machgen_shard.json.gz", "rt", encoding="utf-8") as gz_file:
    ...
    while True:
        try:
            ...
            machgen_paths = [f"machgen_jsonls/{p}" for p in machgen_jsonls]
            ...
            upload_to_gcs(
                ...
                destination_blob_name=f"fulldata/{shard_jsonl}",
            )
            qm.delete(message)
