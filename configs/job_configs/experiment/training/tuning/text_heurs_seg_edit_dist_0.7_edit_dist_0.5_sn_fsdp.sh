...
MODEL_SIZE="medium"
LEARNING_RATE=2.5e-4 # <-- might need to tune
LR="25e5" # <-- might need to tune
AUDIO_HOURS=440
BATCH_SIZE=8
GPU_COUNT=8
REPLICAS=1
...
CLUSTER="ai2/rhea-cirrascale" # can be changed
CLUSTER_NAME="rhea"
HARDWARE="A6000" # <-- can be changed
...
JOB_TYPE="tuning"
SAMPLES_DICTS_DIR="/weka/huongn/training_data/filtered/filtered_from_tagged/text_heurs_seg_edit_dist_edit_dist/text_heurs_seg_edit_dist_0.7_edit_dist_0.5/440K_2" # <-- can be changed (threshold filter)
EPOCH_STEPS=103064 # <-- change depending on dataset
...
VAL_CACHE_DIR="/weka/huongn/ow_val_cache/"
VAL_BATCH_SIZE=16
RUN_VAL=True
VAL_FREQ=500
VAL_WANDB_LOG=True
...
DATE=$(date +"%m%d%y")
EXP_NAME="${EXP_NAME}_${DATE}_${CLUSTER_NAME}"
...
gantry run \
  --name ${EXP_NAME} \
  --task-name ${EXP_NAME} \
  --description "tuning experiment w/ ${LEARNING_RATE}" \
  --allow-dirty \
  --not-preemptible \
  --workspace ${WORKSPACE} \
  --cluster ${CLUSTER} \
