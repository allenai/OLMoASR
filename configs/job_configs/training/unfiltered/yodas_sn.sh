...
EXP_NAME="YODAS_${MODEL_SIZE}_${LR}_${AUDIO_HOURS}K_bs${BATCH_SIZE}_ebs${EFFECTIVE_BATCH_SIZE}_${NUM_WORKERS}workers_${EPOCHS}pass"
...
JOB_TYPE="yodas"
EPOCH_STEPS=44784 # <-- change depending on dataset
...
SCRIPT="scripts/training/prod/train_yodas.py"
...
gantry run \
  --name ${EXP_NAME} \
  --task-name ${EXP_NAME} \
  --description "yodas training experiment" \
  --allow-dirty \
  --not-preemptible \
  --workspace ${WORKSPACE} \
  --cluster ${CLUSTER} \
  ...
  -- /bin/bash -c "torchrun --nnodes ${REPLICAS}:${REPLICAS} --nproc_per_node ${GPU_COUNT} ${SCRIPT} \
      ...
      --eval_dir=/ow_eval \
      --run_id_dir=/weka/huongn/ow_run_ids/yodas \
      --lr=${LEARNING_RATE} \
      --betas='(0.9, 0.98)' \
      --eps=1e-6 \
      --weight_decay=0.1 \
