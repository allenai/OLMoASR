gantry run \
  --name "unfiltered_tiny_15e4_440K_bs40_ebs320_5pass_112424" \
  --description "Training run w/ unfiltered data" \
  --allow-dirty \
  --no-nfs \
  --preemptible \
  ...
  -- /bin/bash -c "torchrun --nnodes 1 --nproc_per_node 8 scripts/training/train.py \
      --model_variant=tiny \
      --exp_name=unfiltered_tiny_15e4_440K_bs40_ebs320_5pass_112424 \
      --job_type=filtering \
      --samples_dicts_dir=/weka/huongn/samples_dicts/unfiltered_all_rnds \
      --train_steps=838861 \
      --epoch_steps=165104 \
      --ckpt_file_name=None \
